...
`A1D3Sqed2RGKAlgebra` / the even-rung `U1A1DevenSqedRGKAlgebra`, gauging
...
e.g. `[S_RG]_2 = c_2·w_2 + (c_1²−c_2)·w_0` on `X_{(0,2)}` (exact `HabiroElement`
coefficients via `sunf_dilog.eq_coeff`).
...
Clebsch–Gordan), orthonormal, spine-free.  It **agrees with the U(2) N_f=1
index through q¹⁰** and differs at **q¹²** (0 vs 2) — the genuine
**SU(2) N_f=1 ⊂ U(2) N_f=1** subalgebra / global-form difference (the two
theories are almost the same, one algebra a subalgebra of the other), *not* a
...
def _c(m: int):
    """The `E_𝖖` coefficient `c_m` (an exact `HabiroElement`), or `None` for
    `m < 0` (a missing dilog factor — the convention `c_{<0} = 0`)."""
    if m < 0:
        return None
    return eq_coeff(m)
