...
Validation lives in `tests/test_finite_zoo_traces.py`: the pentagon
...
regenerate via `experiments/a3_refreeze_from_characters.py`, NOT via
`freeze`).  When (re)generating a5/a1d3/a1d5 — or consuming their
...
* e6 — 22 s for Tr(1) and ~200 s per mg seed at K=8 ALONE; K≳16 is an
  ...
  untimed but comparable or worse.  Run these as background jobs, e.g.::

      PYTHONPATH=. python -c "from elem_traces \
          import freeze; freeze(['e6'], 16)"
...
def _bps_oracle(short_id: str):
    ...
    if short_id not in _ORACLES:
        raise NotImplementedError(
            "spine-free cone export: the BPS oracle is unavailable. Tr(1) "
...
def _vacuum_rps(short_id: str, K: int):
    ...
    (unavailable in the spine-free export — tabulated theories never reach it)."""
...
def freeze(short_ids: list[str], K: int | dict, *,
    ...
    sequentially.  `tests/test_finite_zoo_traces.py::
    test_expected_tables_frozen` guards the expected population."""
    import os
    import pprint
    ...
    with open(path, "w") as f:
        f.write('"""Frozen exact elementary traces for the finite zoo.\n'
                ...
                "su(2)_{-4/3} closed-form characters (regenerate via\n"
                "`experiments/a3_refreeze_from_characters.py`) — the\n"
