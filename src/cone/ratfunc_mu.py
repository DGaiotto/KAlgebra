...
A `RatFuncMu` represents an element of `Q(μ)` (Laurent in q) as
...
solve: every anchor is built as an exact `Q(μ)`-combination of the
...
class RatFuncMu:
    ...
    def __init__(self, shift: int, num: dict, den: dict):
        ...
        den = _p_trim(den)
        if not den:
            raise ZeroDivisionError("RatFuncMu: zero denominator")
        if not num:
    ...
    @staticmethod
    def zero() -> "RatFuncMu":
        return RatFuncMu(0, {}, {0: Fraction(1)})

    @staticmethod
    def one() -> "RatFuncMu":
        return RatFuncMu(0, {0: Fraction(1)}, {0: Fraction(1)})

    @staticmethod
    def from_laurent(lp) -> "RatFuncMu":
        """From a `LaurentPoly` (dict-of-coeffs) -> RatFuncMu (den = 1)."""
        coeffs = lp._coeffs if hasattr(lp, "_coeffs") else lp
        if not coeffs:
            return RatFuncMu.zero()
        return RatFuncMu(0, {k: Fraction(v) for k, v in coeffs.items()},
                       {0: Fraction(1)})
    ...
    # -- arithmetic --
    def __add__(self, o: "RatFuncMu") -> "RatFuncMu":
        if self.is_zero():
            return o
        if o.is_zero():
            return self
        ...
        return RatFuncMu(s, num, den)

    def __neg__(self) -> "RatFuncMu":
        return RatFuncMu(self.shift, _p_scale(self.num, Fraction(-1)), self.den)

    def __sub__(self, o: "RatFuncMu") -> "RatFuncMu":
        return self + (-o)

    def __mul__(self, o: "RatFuncMu") -> "RatFuncMu":
        if self.is_zero() or o.is_zero():
            return RatFuncMu.zero()
        return RatFuncMu(self.shift + o.shift,
                       _p_mul(self.num, o.num), _p_mul(self.den, o.den))

    def __eq__(self, o) -> bool:
        if not isinstance(o, RatFuncMu):
    ...
    def __repr__(self) -> str:
        return f"RatFuncMu(q^{self.shift} * {self.num} / {self.den})"
