...
v-tower + matter match the reference tables through K=4 (216/216) and
K=6 (384/384).
...
def solve_matter(A, known_qn, K, *, margin=4, nmax=3, full=False,
    ...
    The default path generates deep single-ray powers `Tr(ray^p)`
    (`p` up to ≈`Ks+8`, reach-capped), computed per `(i, m, p)` directly — correct
    at all `k`.  (An X01-equivariance shortcut — reduce at `m=0`, shift to all
    `m` — is *false* for `k≥2`, where X01's cocycle with the rays makes the
    dressed reduction differ from a pure seed shift; it is not used.)  The deep
    ...
    cross-pairs (capping the reduction degree to avoid the deep-power degree
    wall) with a degree-stability certificate; see `_solve_matter_smart`."""
...
def _stable(out, prev, K):
    ...
    pinned — `_seed_trace` raises on them rather than silently degrading.)"""
    def norm(d):
...
def _solve_matter_smart(A, known_qn, K, *, margin=4, nmax=3, verbose=False):
    """Degree-first matter bootstrap (avoiding the deep-power degree wall).
    ...
    **total cone-degree** `p1+p2` only until the result is **degree-stable**.
    The reductions stay low-degree (cheap), avoiding deep single-ray
    ...
    `(i,j,p1,p2,m1,m2)`).  A shift-based generation (reduce at `m=0`,
    X01-shift to all `m`) would be **wrong for k≥2** — `_pair_poly` is
    X01-equivariant only at k=1 (X01's cocycle with the rays vanishes there),
    so the shift produces false constraints (visible as an inconsistency at
    𝖖¹); each dressed pair is therefore computed directly.

    ⚠ STATUS: **experimental; not the default path.**  The concept
    is *validated correct* at k=1 (degree-stable; matter seeds match the frozen
    reference tables).  The proven takeaway: **degree-capping beats the
    deep-power wall**
    (`seed_reduction(ray^p)` is 0.4s at p=4 but >100s at p=6 for k=2 — the k=2
    stall).  Not the production path because the direct `m1×m2` generation
    over-generates (`rays²·m1·m2` per degree); a lean, selective generator (one
    good constraint per (seed, order), single sweep) would be needed to make
    k=2 fast."""
    tb = A._tb
    Ks = K + margin
    mag0 = _matter_seeds(A)
