...
* `compute_S_ket`, `apply_F_to_state`, `eq_coefficients`, the old
  `schur_index()` function, and `nahm_inner_product` — moved to
  `legacy/schur_index_state.py`.  The old PowerSeries-throughout pipeline
  is preserved for regression comparisons only; it is known to fail for
  deep-support F_a (pentagon `(-3,2)` etc. -- see CLAUDE.md).
