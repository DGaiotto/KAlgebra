"""
u1_decagon_cone_data.py
=======================

U1DecagonConeData(FiniteConeData) — k=3 octagon-style wiring.

Primitives: L_1 (10) + L_2 (10) + L_3 (10) + E^± = 32 mult-gens.
Cone data derived from MULT_TABLE (= u1_decagon_mult_table.py).
"""
from __future__ import annotations
from cone_data import FiniteConeData, CrossProductTerm, Cone
from laurent_poly import LaurentPoly
from u1_decagon_mult_table import MULT_TABLE


# E_GEN / E_INV — encoded as (4, 0) / (4, 1) so default sorted() works
# and doesn't collide with L_1/L_2/L_3 (which use family indices 1, 2, 3).
E_GEN = (5, 0)
E_INV = (5, 1)


def _all_chord_gens():
    return [(a, i) for a in (1, 2, 3) for i in range(10)] + [(4, i) for i in range(5)]


def _all_mult_gens():
    return _all_chord_gens() + [E_GEN, E_INV]


def _is_L_letter(g):
    return g[0] in (1, 2, 3, 4)


# MU_LETTER_QPOWER: cocycle c(L_l, E).
#   L_1 (magnetic): alternating ±1 by parity
#   L_2 (non-magnetic): 0
#   L_3 (magnetic): alternating ±1 by parity
#   L_4 (non-magnetic, NEW): 0 (verified)
MU_LETTER_QPOWER = {}
for i in range(10):
    MU_LETTER_QPOWER[(1, i)] = -1 if i % 2 == 0 else +1
    MU_LETTER_QPOWER[(2, i)] = 0
    MU_LETTER_QPOWER[(3, i)] = -1 if i % 2 == 0 else +1
for i in range(5):
    MU_LETTER_QPOWER[(4, i)] = 0


def _native_label_to_word(native_label):
    factors, e_E = native_label
    word = []
    for (a, i, exp) in sorted(factors):
        word.extend([(a, i)] * exp)
    if e_E > 0:   word.extend([E_GEN] * e_E)
    elif e_E < 0: word.extend([E_INV] * (-e_E))
    return tuple(word)


class U1DecagonConeData(FiniteConeData):
    def __init__(self):
        self._mult_gens = tuple(_all_mult_gens())
        # (a, i) chord generators — read by the orthonormality-bootstrap
        # driver (`u1aodd_trace_bootstrap.solve_intermediate`).
        self._chords = _all_chord_gens()
        self._cones = None

    def mult_gens(self):
        return self._mult_gens

    def cones(self):
        if self._cones is None:
            V = list(self._mult_gens)
            neighbours = {
                v: frozenset(u for u in V if u != v and self.q_commute(v, u))
                for v in V
            }
            cliques = []

            def bk(R, P, X):
                if not P and not X:
                    cliques.append(R); return
                pivot = max(P | X, key=lambda u: len(P & neighbours[u]))
                for v in list(P - neighbours[pivot]):
                    bk(R | {v}, P & neighbours[v], X & neighbours[v])
                    P = P - {v}; X = X | {v}

            bk(frozenset(), frozenset(V), frozenset())
            self._cones = tuple(cliques)
        return self._cones

    def q_commute(self, g, h):
        if g == h: return True
        # QTCone (Laurent E): E and E⁻¹ q-commute with cocycle 0.
        # The cancellation E · E⁻¹ = 1 is handled by the universal
        # `_torus_inverse_letter` collapse in
        # `ConeData._word_to_gens_powers`, not via cross_product.
        if {g, h} == {E_GEN, E_INV}: return True
        if _is_L_letter(g) and _is_L_letter(h):
            fwd = MULT_TABLE.get((g, h))
            return fwd is not None and len(fwd) == 1
        return True  # L-letter ↔ E always q-commute

    def cocycle(self, g, h):
        if g == h: return 0
        # E and E⁻¹ commute (cocycle 0) — torus direction.
        if {g, h} == {E_GEN, E_INV}: return 0
        if _is_L_letter(g) and _is_L_letter(h):
            fwd = MULT_TABLE.get((g, h)); bwd = MULT_TABLE.get((h, g))
            if fwd is None or len(fwd) != 1 or bwd is None or len(bwd) != 1:
                raise ValueError(f"cocycle: L{g}, L{h} not q-commuting")
            c_full = fwd[0][0] - bwd[0][0]
            assert c_full % 2 == 0
            return c_full // 2
        if g == E_GEN and _is_L_letter(h): return -MU_LETTER_QPOWER[h]
        if h == E_GEN and _is_L_letter(g): return MU_LETTER_QPOWER[g]
        if g == E_INV and _is_L_letter(h): return MU_LETTER_QPOWER[h]
        if h == E_INV and _is_L_letter(g): return -MU_LETTER_QPOWER[g]
        raise ValueError(f"cocycle undefined for ({g}, {h})")

    def cross_product(self, g, h):
        # Note: (E_GEN, E_INV) no longer reaches here — q-commute with
        # cocycle 0; cancellation via `_torus_inverse_letter`.
        if {g, h} == {E_GEN, E_INV}:
            return [(LaurentPoly.one(), ())]
        if _is_L_letter(g) and _is_L_letter(h):
            terms = MULT_TABLE.get((g, h))
            if terms is None:
                raise ValueError(f"MULT_TABLE missing ({g}, {h})")
            out = []
            for (q_pow, e_E, factors, q_coef) in terms:
                word = list(factors)
                if e_E > 0:   word.extend([E_GEN] * e_E)
                elif e_E < 0: word.extend([E_INV] * (-e_E))
                gens_set = frozenset(word)
                powers = {gx: word.count(gx) for gx in gens_set}
                phase = self.cone_label_phase(gens_set, powers)
                coef = q_coef * LaurentPoly({q_pow + phase: 1})
                out.append((coef, tuple(word)))
            return out
        raise ValueError(f"cross_product undefined for q-commuting ({g}, {h})")

    def to_cone_label(self, native_label):
        word = _native_label_to_word(native_label)
        if not word:
            for c in self.cones():
                if E_GEN in c: return (c, {g: 0 for g in c})
            return (self.cones()[0], {g: 0 for g in self.cones()[0]})
        gens_in_word = set(word)
        for cone in self.cones():
            if gens_in_word <= set(cone):
                powers = {g: word.count(g) for g in cone}
                return (cone, powers)
        raise ValueError(f"no cone contains all letters of {native_label}: {gens_in_word}")

    def from_cone_label(self, gens, powers):
        factors_dict = {}; e_E = 0
        for g, p in powers.items():
            if p == 0: continue
            if g == E_GEN: e_E += p
            elif g == E_INV: e_E -= p
            else:
                a, i = g
                factors_dict[(a, i)] = factors_dict.get((a, i), 0) + p
        factors = tuple(sorted((a, i, p) for (a, i), p in factors_dict.items()))
        return (factors, e_E)

    # -- torus inverse letter (Laurent E direction; QTCone semantics) -----

    def _torus_inverse_letter(self, g):
        """E ↔ E⁻¹ pairing.  Universal cancellation collapse in
        `ConeData._word_to_gens_powers` replaces the previous
        cross_product Plücker handling."""
        if g == E_GEN: return E_INV
        if g == E_INV: return E_GEN
        return None

    # -- QTCone iter ------------------------------------------------------

    def iter_cones(self):
        """Yield QTCone instances per maximal cone, with
        `torus_gens = {E, E⁻¹} ∩ cone_gens`."""
        for cone_gens in self.cones():
            torus_in_cone = frozenset({E_GEN, E_INV}) & cone_gens
            yield Cone(self, cone_gens, torus_gens=torus_in_cone)


if __name__ == "__main__":
    D = U1DecagonConeData()
    print(f"U1DecagonConeData: {len(D.mult_gens())} mult-gens")
    cones = D.cones()
    print(f"  {len(cones)} maximal cones (Bron-Kerbosch)")
    from collections import Counter
    print(f"  Cone sizes: {dict(sorted(Counter(len(c) for c in cones).items()))}")
